<script>
	...
	*/
	...
	function fetchList() {
		return API.graphql(graphqlOperation(listTodos)).then(({ data }) => {
			...
			// console.log({data, todos}); // return void
		});
	}
	onMount(fetchList);
	...
	*/
	function add(input) {
		...
		input.value = "";
		return API.graphql(graphqlOperation(createTodo, { input: todo })).then(
			fetchList
		);
	}
	/*
	...
	*/
	...
	/*
	UPDATE - move an item from todo to done and back
	*/
